FireDragonSnap = InheritsFromBaseAttackSkill();

function FireDragonSnap:Update()  
  local me = FightField.fightHum[self.humIdx];
  local target = FightField.fightHum[self.tarIdx];

  if self.step == 1 then           
    FightField.attack[self.humIdx]:SetLightWithOrder(1, EEffectLightDis.Sky, 10291, 20, 4, 18, 18, EEffectLightTracer.StandAni, me.nowPos.x, me.nowPos.y + 60, 1, true, 30);
    self.interval = 0.1;
    ---2--------------------------------------------------------------
  ...
  elseif self.step == 2 then
    if self:CheckInterval() then 
      ...
      FightField.attack[self.humIdx]:SetLightWithOrder(1, EEffectLightDis.Sky ,10291, 20, 4, 18, 18, EEffectLightTracer.Curve, me.nowPos.x + 30, me.nowPos.y + 60, 40, true, 0, target.nowPos.x, target.nowPos.y + 60, 0.66);
  ...
  elseif self.step == 3 then
    if me.arrive then
      FightField.attack[self.humIdx]:SetLightWithOrder(1, EEffectLightDis.Sky ,10292, 40, 1, 12, 12, EEffectLightTracer.StandAni, me.nowPos.x, me.nowPos.y + 60, 1, true, 30);
...
EW_FireDragonSnap = InheritsAttack(FireDragonSnap);

function EW_FireDragonSnap:InitDrawBlack()
