QuakeWater = InheritsFromBaseAttackSkill();

function QuakeWater:InitExtraParameters()
  ...
  self.UpdateDragon = false;
end

--穫得攻擊目標idx 函式-------------------------------------------
function QuakeWater:GetAttackIdx(count)
...
function QuakeWater:PlayEffectLight()
  local target = FightField.fightHum[self.aimIdx]
  local me = FightField.fightHum[self.humIdx]
  if target.party_Kind == EFightParty.Left then
    if self.attackStep == 1 or self.attackStep == 3 then
      FightField.attack[self.humIdx]:SetLightWithOrder(1 , EEffectLightDis.Sky,10432, 30, 1, 5, 5, EEffectLightTracer.StandAni, me.nowPos.x - 5, target.nowPos.y, 32, false, 40);      
    elseif self.attackStep == 2 then
      FightField.attack[self.humIdx]:SetLightWithOrder(1, EEffectLightDis.Sky ,10433, 30, 1, 5, 5, EEffectLightTracer.StandAni, me.nowPos.x, target.nowPos.y, 32, false, 40);
    end
  else
    if self.attackStep == 1 or self.attackStep == 3 then
      FightField.attack[self.humIdx]:SetLightWithOrder(1, EEffectLightDis.Sky,10433, 30, 1, 5, 5, EEffectLightTracer.StandAni, me.nowPos.x, target.nowPos.y, 32, false, 40);
  ...
  FightField.attack[self.humIdx].lights[1]:SetColor(Color.LightBlue);

  FightField.attack[self.humIdx]:SetLightWithOrder(2, EEffectLightDis.Sky, 10328, 30, 1, 7, 7, EEffectLightTracer.StandAni, target.nowPos.x, target.nowPos.y, 30, false, 50);
  FightField.attack[self.humIdx]:SetLightWithOrder(3, EEffectLightDis.Sky, 10312, 30, 1, 14, 14, EEffectLightTracer.StandAni, target.nowPos.x, target.nowPos.y, 30, false, 40);  
  FightField.attack[self.humIdx]:SetLightWithOrder(4, EEffectLightDis.Sky, 20032, 70, 1, 11, 11, EEffectLightTracer.StandAni, target.nowPos.x, target.nowPos.y + 100, 32, false, 0);
  self.UpdateDragon = true;
end

function QuakeWater:UpdateDragonEffect()
  local target = FightField.fightHum[self.aimIdx]
  if target ~= nil and self.UpdateDragon and FightField.attack[self.humIdx].lights[4].picId == 0 then     
    FightField.attack[self.humIdx]:SetLightWithOrder(4, EEffectLightDis.Sky, 20033, 70, 1, 10, 10, EEffectLightTracer.StandAni, target.nowPos.x, target.nowPos.y + 100, 32, false, 0); 
    self.UpdateDragon = false;
  end
end